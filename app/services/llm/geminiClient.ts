...
// agents-ai's client returns English error strings (and runs metering — #237
// — under the hood); this app is Hebrew-only, so map its known error shapes
// back to the same Hebrew messages the hand-rolled fetch() used to produce.
// Downstream (chatProcessor.ts) only checks `.error` for truthiness, not
// content, so exact string parity isn't load-bearing — this is purely to
// keep Hebrew consistent wherever `.error` does surface.
