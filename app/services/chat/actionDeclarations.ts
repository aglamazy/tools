...
export const ACTION_DECLARATIONS: FunctionDeclaration[] = [
  {
    name: 'find_setting',
    description: 'מצא איפה במערכת מוגדרת/נמצאת הגדרה או מסך מסוים, ונווט לשם. השתמש כששואלים "איפה מוגדר X" / "איפה אני משנה את X" / "קח אותי ל-X", וגם כששואלים "מה זה X" / "מה המשמעות של X" כאשר X עשוי להיות מונח, מסך או הגדרה ספציפיים לאפליקציה הזו (לא מושג פיננסי/כלכלי כללי כמו "ריבית דריבית" או "פחת") — לגבי הגדרות, מסכי עסק, או מסכי האפליקציה הראשיים (לא מוצרי מכולת). אל תנחש נתיב או תשובה מהזיכרון — אם לא בטוח האם X קיים באפליקציה, קרא לכלי הזה קודם.',
    parameters: {
      type: 'object',
      properties: {
