...
export interface YpayDocument {
  ...
  paidAt?: string // ISO timestamp when an INVOICE (104/106) was fully closed off by its receipt(s) — not the same as moneyReceivedAt below
  moneyReceivedAt?: string // YYYY-MM-DD — when the money actually landed in the bank, for settlement/partner-split math. Distinct from createdAt (the document's own issue date in ypay, which drives which tax period it's declared in) — Agla, aglamazo#381: "the first is when I created the document in ypay, the second is when the money come." Unindexed, additive field — no schema version bump needed. Unset on a document with no known real-money date (an invoice not yet paid, or an import that couldn't be matched to a bank transaction) — never guessed.
